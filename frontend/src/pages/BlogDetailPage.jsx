...
const BlogDetailPage = () => {
  ...
  useEffect(() => {
    if (socket && id) {
      ...
      return () => {
        socket.emit("leave_blog_post", id);
        socket.off("comment_added");
        socket.off("vote_changed");
      };
    }
  }, [socket, id]);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    if (user && user._id) {
      ...
      if (newComment) {
        ...
        if (socket) {
          socket.emit("new_comment", { postId: id, commentId: newComment._id });
        }
      }
      setCommentContent("");
    }
  ...
  const handleReplySubmit = async (e, parentId, replyingToId) => {
    ...
    if (user && user._id && parentId) {
      ...
      if (newReply) {
        ...
        // Emit socket event for real-time update
        if (socket) {
          socket.emit("new_comment", { postId: id, commentId: newReply._id });
        }
        setReplyingTo(null);
        setReplyContent("");
      }
  ...
  const handleUpvote = async () => {
    if (user && user._id && !hasUpvoted) {
      ...
      // Emit socket event for real-time update
      if (socket) {
        socket.emit("vote_update", { postId: id, type: "upvote" });
      }
    }
    if (!user) {
      toast.error("Please Sign In to Perform Action");
  ...
  const handleDownvote = async () => {
    if (user && user._id && !hasDownvoted) {
      ...
      // Emit socket event for real-time update
      if (socket) {
        socket.emit("vote_update", { postId: id, type: "downvote" });
      }
    }
    if (!user) {
      toast.error("Please Sign In to Perform Action");
